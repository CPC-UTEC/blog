...
Usually, when solving a problem finding the big oh of a function is a simple as simplifying it to its highest order term. However, a more formal definition of big oh exists:

$ f(n) = O(g(n)) \rightarrow f(n) \leq kg(n)$ for $n > n\_0$ and $k > 0$

This definition says that a function $f(n)$ is considered to have a big oh of $g(n)$ if there exists a value of $k$ such that $kg(n)$ will be greater than $f(n)$ from some value $n_0$ of $n$ onwards. From this definition it is important to note some things:
