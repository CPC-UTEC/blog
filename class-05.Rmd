...
There are some kind of problems when you have some equations on $m$ variables.
Then, its usually a good idea to first try a brute force approach. Find the range of
each of the variables, do a loop for each of them, then inside each loop you
have the value of that variable fixed, now its a constant. Moreover, when you
have fixed $m - 1$ of these variables its usually easy to compute what the last
...
force solution with two loops can solve the problem. Now, in order two choose
what variable two fix, always prefer working with integers (you'll understand
...
Lets say the maximum value we will compute is $L$, then $i, j, k, l = O(\log L)$. So, if we fix $L$
we can compute the elements of $S$ that are less than $L$ in $O(\log^4(L))$. We
can simply try different values of $L$ until we get more number than the
...
At first glance, the problem looks difficutl, yet we have
...
because of a property or that there one of the variables of the input is small
and we can try a brute force approach using it.

<div class="topic">Case analysis</div>

There are some problem that looks difficult, yet when you analyze the cases
...
**Problem:** You are given an array $a$ of $n$ elements. Output two numbers $x, y \mid a_i \not = a_j \land i < j$ such that when swapping $a_i$ and  $a_j$ the array is not sorted (increasing or decreasing) or output -1 if no such pair or number exits.
...
* Case 3: There are exactly 2 different elements.
  ...
  in decreasing order we have and answer, else the array will be of the form:

  $$a_1 = y \leq a_2 = y \leq \dots \leq a_i = y \leq \dots \leq a_j = x \leq \dots a_n =x$$

  So, if there exits ans answer it must be in $[i, j)$ or $(i, j]$. Then, we
  ...
  in increasing order we have and answer, else the array will be of the form:

  $$a_1 = x \leq a_2 = x \leq \dots \leq a_i = x \leq \dots \leq a_j = y \leq \dots a_n =y$$
...
So, we can compute $prefix$ in an array and answer each query in $O(1)$.
