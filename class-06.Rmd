...
the other one decreasing. In practice, intead of using a pointer, a `int`
...
minimun number of intervals $[l_i, r_i] \mid a_i = a_j \forall \, i, j \in [l_i,
...
**Problem (2 sum problem):** You are given an array $a$ of $n$ positive elements and an integer $x$. You
should output two numbers $i, j \mid i \not = j \land a_i + a_j = x$. It is garantee that a solution always exits.

$$1 \leq n \leq 10^5$$
$$1 \leq x \leq 10^9$$

We can use the same technique and solve the problem in $O(n)$.
...
How could you solve the problem if you need to find three numbers $x, y, z \mid
...
<div class=topic><a href="https://vjudge.net/contest/353470">Contest</a></div>
