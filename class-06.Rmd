...
the $n$ disks that are is the `source` rod to the `target` rod. Then, we can
say:
...
direcly move to `target`. Then, we can write $f$ in this way:
...
$r$ is what we get when we compute $a \% b$.
...
For example, if $m = 12 \to x = 20$ would be represented as $x \mod 12 = 8$.
...
If $m$ is prime ans $0 < a < m$, we have:
