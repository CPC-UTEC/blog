...
From these we notice that $f(x) = L \leftrightarrow L \equiv 0 \bmod 4$. Then, we have
solved the problem.
...
$$g(state) = mex\{g(state') \quad \forall state' \text{ reachable
from } state \}$$
...
Now, we are solving each case in $O(nm \log m)$ which is enough to get the accepted veredict.
