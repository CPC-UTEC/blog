...
The function $mex$ is known as the **Spragre-Grundy function** or as **minimum excludant** and its values as **grundy numbers** or **Nim values**. And what is nice about this function is that $mex(state) = 0 \leftrightarrow states \text{ is a losing state}$.
...
Now, we are solving each case in $O(nm \log m)$ which is enough to get Accepted veredict.
...
and Grundy numbers. Now, what happend if we form a game as the sum of several
 impartial combinatorial games ? Well, in this case this theorem comes in
 handy:
...
Now, let's see an example of this theorem.
...
the grundy number of the position of the knights.
...
We may have also solve the problem using WL states or just Grundy numbers, but
